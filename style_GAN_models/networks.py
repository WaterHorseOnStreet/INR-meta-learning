...
import numpy as np
from modules import ResnetBlock, CondInstanceNorm, TwoInputSequential, CINResnetBlock, InstanceNorm2d, FCNBlock,TwoInputModule
...
def define_G(input_nc, output_nc, ngf=64, norm='instance', which_model_netG='resnet',
...
def define_stochastic_G(nlatent, input_nc, output_nc, ngf=64, norm='instance',
                        which_model_netG='resnet', use_dropout=False, use_gpu=True):
...
class InitialDeconvolution(nn.Module):
    def __init__(self, input_nc = 1, output_nc =16, norm_layer=nn.BatchNorm2d):
        super(InitialDeconvolution, self).__init__()

        self.input_nc = input_nc
        self.output_nc = output_nc
        model = []
        model += [nn.ConvTranspose2d(4, 100, kernel_size=(1,1), stride=1, padding=0)]
        self.model = nn.Sequential(*model)

    def forward(self, input):
        return self.model(input)

def define_InitialDeconv(init_type='normal'):
    net_Deconv = None
...
class Discriminator(nn.Module):
    def __init__(self,in_channles,resolution,label_size,channel_table,num_layers=18):
        ...
        for block in range(self.nblocks):
            if block == 0:
                layers.append(self.build_basic_block(self.in_channels,self.channel_table[block]))
                layers.append(self.build_basic_block(self.channel_table[block],self.channel_table[block],down_sample=True))
            else:
                layers.append(self.build_basic_block(self.channel_table[block-1],self.channel_table[block]))
                layers.append(self.build_basic_block(self.channel_table[block],self.channel_table[block],down_sample=True)) 

        self.Linear_map = nn.Linear(self.resolution*self.resolution*self.channel_table[-1],label_size,bias=True)
    ...
    def build_basic_block(self,in_channel,out_channel,down_sample=False):
            layers = []
            if down_sample:
...
class MappingNetwork(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, label_dim,num_layers=8):
        super().__init__()
        ...
        for i in range(num_layers):
            if i == 0:
                layers.append(FCNBlock(in_dim=in_dim,out_dim=hidden_dim))
            elif i == num_layers - 1:
                layers.append(FCNBlock(in_dim=hidden_dim,out_dim=out_dim))
            else:
                layers.append(FCNBlock(in_dim=hidden_dim,out_dim=hidden_dim))
...
class SyntheticNet(nn.Module):
    ...
    def build_synthetic_block(self,layer_idx,up_sample=False):
        if layer_idx == 0:
            layers = []
            layers.append(nn.Conv2d(self.in_channels,self.channel_table[layer_idx],kernel_size=3,padding=1))
            layers.append(nn.BatchNorm2d(self.channel_table[layer_idx]))
            layers.append(nn.ReLU())
            return nn.Sequential(*layers)
        else:
            layers = []
            if up_sample:
                ...
                layers.append(nn.Conv2d(self.channel_table[layer_idx-1],self.channel_table[layer_idx],kernel_size=3,padding=1))
            else:
                layers.append(nn.Conv2d(self.channel_table[layer_idx],self.channel_table[layer_idx],kernel_size=3,padding=1))
            layers.append(nn.BatchNorm2d(self.channel_table[layer_idx]))
            layers.append(nn.ReLU())
            return nn.Sequential(*layers)
...
class StyleAdder(TwoInputModule):
    def __init__(self,in_dim,out_dim,layer_idx):
        ...
        self.affine = FCNBlock(in_dim=in_dim,out_dim=out_dim,activation=None)
        self.idx = layer_idx
