...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        if cblock is classifier.MLPBlock:
            self.classifier_class.append(nn.AdaptiveAvgPool2d((7, 7)))
            self.classifier_class.append(cblock(7 * 7 * 512 * block.expansion, num_classes, **block_kwargs))
        else:
            self.classifier_class.append(cblock(self.feature_split[0] * block.expansion, num_classes, **block_kwargs))
        self.classifier_class = nn.Sequential(*self.classifier_class)

        self.classifier_domain = []
        if cblock is classifier.MLPBlock:
            self.classifier_domain.append(nn.AdaptiveAvgPool2d((7, 7)))
            self.classifier_domain.append(cblock(7 * 7 * 512 * block.expansion, 3, **block_kwargs))
        else:
            self.classifier_domain.append(cblock(self.feature_split[1] * block.expansion, 3, **block_kwargs))
        self.classifier_domain = nn.Sequential(*self.classifier_domain)
    ...
    def forward(self, x):
        ...
        feature_for_class = x[:,:self.feature_split[0],:,:]
        feature_for_domain = x[:,self.feature_split[0]:,:,:]

        class_pred = self.classifier_class(feature_for_class)
        domain_pred = self.classifier_domain(feature_for_domain)

        x = torch.cat((class_pred,domain_pred),dim=1)
